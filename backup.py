...
def parallel_tuning_process(spawn_count, kfolds, X, y):
    ...
    expression_list = [(X, y, tuning_params, partitions, k) for k in range(0, kfolds)] 

    process_jobs = []

    for i in range(1, spawn_count + 1):
        p = multiprocessing.Process(target = cvkfold, args = (X, y, tuning_params, partitions, kfolds - 1))
...
if __name__=='__main__':
    ...
    spawn_count = 1															# number of processes to spawn
    kfolds = 5
    t1 = time()
    print(t1)
